...
def findCards(orig,gray, game):
    ...
    if orig.any():
        ...
        for(i, c) in enumerate(cnts):
            ...
            if locs:
                game.addToHand(CVBoard.addFromBoard(orig,locs,gray)) #Send roi region (should be the name) to be described
                #time.sleep(1)
                locs = []
...
def main(): 
    while(True):
        ...
        findCards(roi_board, roi_board_gray, game)
